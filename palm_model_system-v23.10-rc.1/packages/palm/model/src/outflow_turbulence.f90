...
! Description:
! ------------
!> Routine based on inflow_turbulence.f90. Copies values of 3d data from a 2d vertical source plane
!> (defined by outflow_source_plane) to the outflow boundary.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE outflow_turbulence
    ...
    REAL(wp), DIMENSION(nzb:nzt+1,nysg:nyng,7,nbgp) ::  outflow_val  !< values to be copied to the outflow boundary
...
!-- For parallel runs, send the values to the respective outflow PE
    ...
    IF ( myidx == id_outflow_source  .AND.  myidx /= id_outflow )  THEN
       ...
       CALL MPI_SEND( outflow_val(nzb,nysg,1,1), ngp_ofv, MPI_REAL, id_outflow, 1, comm1dx, ierr )

    ELSEIF ( myidx /= id_outflow_source  .AND.  myidx == id_outflow )  THEN

       outflow_val = 0.0_wp
       CALL MPI_RECV( outflow_val(nzb,nysg,1,1), ngp_ofv, MPI_REAL, id_outflow_source, 1, comm1dx, &
                      status, ierr )
