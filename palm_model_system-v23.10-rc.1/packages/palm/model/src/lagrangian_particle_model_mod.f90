...
!
!-- So far, vertically shifted child domains do not allow particle nesting.
    IF ( particle_coupling  .AND.  child_domain  .AND.  nest_shift_z /= 0.0_wp )  THEN
       message_string = 'particle_coupling = .T. not allowed for vertically shifted child domains'
