...
!--       dependencies.
          IF ( cthf == 0.0_wp  .AND.  ( .NOT. urban_surface  .AND.  .NOT. land_surface ) )  THEN
             ...
             CALL message( 'pcm_check_data_output', 'PCM0001', 1, 2, 0, 6, 0 )
          ENDIF
          unit = 'K s-1'
       ...
       CASE ( 'pcm_bad', 'pcm_lad' )
          IF ( j == 1 )  THEN
             message_string = 'time averaging of a static quantity ("' // TRIM( var ) //            &
                              '") is not provided'
             CALL message( 'pcm_check_data_output', 'PCM0002', 1, 2, 0, 6, 0 )
          ENDIF
          unit = 'm2 m-3'
...
!--------------------------------------------------------------------------------------------------!
    SUBROUTINE pcm_check_parameters
       ...
       IF ( ocean_mode )  THEN
          ...
          CALL message( 'pcm_check_parameters', 'PCM0003', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( canopy_drag_coeff == 0.0_wp )  THEN
          ...
          CALL message( 'pcm_check_parameters', 'PCM0004', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( ( alpha_lad /= 9999999.9_wp .AND. beta_lad == 9999999.9_wp )  .OR.                     &
          ...
          CALL message( 'pcm_check_parameters', 'PCM0005', 1, 2, 0, 6, 0 )
       ENDIF
       ...
       IF ( calc_beta_lad_profile  .AND.  lai_beta == 0.0_wp )  THEN
          message_string = 'using the beta function for the construction ' //                      &
                           ...
                           'lai_beta = 0.0'
          CALL message( 'pcm_check_parameters', 'PCM0006', 1, 2, 0, 6, 0 )
       ENDIF
...
!> located.
!--------------------------------------------------------------------------------------------------!
 SUBROUTINE pcm_data_output_3d( av, variable, found, local_pf, nzb_do, nzt_do )
...
!--                Check if canopy information is consistent to numeric grid.
                   IF ( leaf_area_density_f%nz > SIZE( zu ) )  THEN
                      ...
                      CALL message( 'pcm_init', 'PCM0009', 2, 2, 0, 6, 0 )
                   ENDIF
                   ...
                   THEN
                      ...
                      CALL message( 'pcm_init', 'PCM0010', 2, 2, 0, 6, 0 )
                   ENDIF
                ELSE
                   message_string = 'Reading canopy data - vertical dimension "zlad" missing.'
                   CALL message( 'pcm_init', 'PCM0011', 2, 2, 0, 6, 0 )
                ENDIF
!
...
#endif
                IF ( lad_on_top )  THEN
                   message_string = 'LAD profile is omitted at building grid points which have' // &
                                    ' been artificially created by the topography filtering'
                   CALL message( 'pcm_init', 'PCM0012', 0, 0, 0, 6, 0 )
                ENDIF
...
#endif
                IF ( bad_on_top )  THEN
                   message_string = 'BAD profile is omitted at building grid points which have' // &
                                    ' been artificially created by the topography filtering'
                   CALL message( 'pcm_init', 'PCM0012', 0, 0, 0, 6, 0 )
                ENDIF
...
!--    radiation model.
       ...
       IF ( lad_max <= 0.0_wp )  THEN
          ...
          CALL message( 'pcm_init', 'PCM0013', 1, 2, 0, 6, 0 )
       ENDIF
...
!--          Check whether topography and local vegetation on top exceed height of the model domain.
             IF ( topo_top_ind(j,i,0) + pch_index_ji(j,i) >= nzt + 1 )  THEN
                ...
                CALL message( 'pcm_init', 'PCM0014', 2, 2, myid, 6, 0 )
             ENDIF
...
!--             plant column must be "lifted", which is done in SUBROUTINE pcm_tendency.
                IF ( pctype < 0  .OR.  pctype > 10 )  THEN   !< incorrect plant canopy type
                   ...
                   CALL message( 'pcm_read_plant_canopy_3d', 'PCM0015', 1, 2, 0, 6, 0 )
                ENDIF
             ...
             CASE DEFAULT
                ...
                CALL message( 'pcm_read_plant_canopy_3d', 'PCM0016', 1, 2, 0, 6, 0 )
          END SELECT
       ENDDO

515    message_string = 'error opening file PLANT_CANOPY_DATA_3D'
       CALL message( 'pcm_read_plant_canopy_3d', 'PCM0017', 1, 2, 0, 6, 0 )

516    message_string = 'error reading file PLANT_CANOPY_DATA_3D'
       CALL message( 'pcm_read_plant_canopy_3d', 'PCM0018', 1, 2, 0, 6, 0 )
...
!--                Determine topography-top index on scalar-grid
          ...
          CASE DEFAULT
             ...
             CALL message( 'pcm_tendency', 'PCM0019', 1, 2, 0, 6, 0 )
