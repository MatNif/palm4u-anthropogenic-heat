...
!------------------------------------------------------------------------------!
 PROGRAM test_transform
    ...
    USE inifor_defs,                                                           &
        ONLY :  TO_RADIANS, TO_DEGREES, wp
    USE inifor_grid,                                                           &
        ONLY :  grid_definition, init_grid_definition
    USE inifor_transform,                                                      &
        ONLY :  phi2phirot, rla2rlarot, phirot2phi, rlarot2rla
    USE test_utils
    ...
    ! Angels in degrees
    REAL(wp) ::  phi, lambda, phi_c, lambda_c, phi2, lambda2, phi_n, lambda_n
    ...
    lambda_c  = rla2rlarot(phi,   lambda, phi_n, lambda_n, 0.0_wp)

    phi2      = phirot2phi(phi_c, lambda_c, phi_n, 0.0_wp)
    lambda2   = rlarot2rla(phi_c, lambda_c, phi_n, lambda_n, 0.0_wp)
